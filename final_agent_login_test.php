...
if ($result['code'] == 302) {
    ...
    if ($result['code'] == 302) {
        ...
        if ($result['code'] == 200) {
            ...
            // Step 4: Select agent role (should redirect to login form)
            echo "4. Selecting agent role...\n";
            $result = makeRequest($baseUrl . '/choose-role/agent', null, $cookieJar);
            echo "Status: {$result['code']}\n";
            echo "Redirect: {$result['redirect']}\n\n";

            if ($result['code'] == 302) {
                echo "5. Following redirect to agent login form...\n";
                $loginFormUrl = $result['redirect'];
                $result = makeRequest($loginFormUrl, null, $cookieJar);
                echo "Status: {$result['code']}\n";
                ...
                if ($result['code'] == 200) {
                    ...
                    if ($csrfToken) {
                        ...
                        echo "6. Submitting agent login credentials...\n";
                        $loginData = [
                            '_token' => $csrfToken,
                        ...
                        $result = makeRequest($loginFormUrl, $loginData, $cookieJar);
                        echo "Status: {$result['code']}\n";
                        echo "Redirect: {$result['redirect']}\n\n";

                        if ($result['code'] == 302) {
                            echo "7. Following post-login redirect...\n";
                            ...
                            if ($result['code'] == 200) {
                                ...
                                if (strpos($result['response'], 'Dashboard Agent') !== false ||
                                    ...
                                    echo "⚠ Warning: May not be the agent dashboard\n";
                                }
                            } else {
                                echo "❌ ERROR: Could not access dashboard after login\n";
                            }
                        } else {
                    ...
                    echo "❌ ERROR: Could not reach login form\n";
                }
            } else {
                echo "❌ ERROR: Role selection failed\n";
            }
        } else {
            echo "❌ ERROR: Could not reach role selection\n";
        }
    } else {
        echo "❌ ERROR: Login redirect failed\n";
    }
} else {
    echo "❌ ERROR: Initial redirect failed\n";
}
