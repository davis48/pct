    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div class="text-center p-4 bg-blue-50 rounded-lg transition-all duration-300" x-bind:class="{'ring-2 ring-blue-300': isUpdating}">
                <p class="text-2xl font-bold text-blue-600" x-text="myStats.my_processed_today">{{ $myStats['processed_today'] ?? 0 }}</p>
            ...
            <div class="text-center p-4 bg-green-50 rounded-lg transition-all duration-300" x-bind:class="{'ring-2 ring-green-300': isUpdating}">
                <p class="text-2xl font-bold text-green-600" x-text="myStats.my_processed_week">{{ $myStats['processed_this_week'] ?? 0 }}</p>
            ...
            <div class="text-center p-4 bg-orange-50 rounded-lg transition-all duration-300" x-bind:class="{'ring-2 ring-orange-300': isUpdating}">
                <p class="text-2xl font-bold text-orange-600" x-text="myStats.my_avg_time">{{ $myStats['avg_processing_time'] ?? 'N/A' }}</p>
            ...
            <div class="text-center p-4 bg-purple-50 rounded-lg transition-all duration-300" x-bind:class="{'ring-2 ring-purple-300': isUpdating}">
                <p class="text-2xl font-bold text-purple-600" x-text="myStats.my_success_rate + '%'">{{ $myStats['success_rate'] ?? 0 }}%</p>
        ...
        <div class="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            <div class="bg-gray-50 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-lg font-bold text-indigo-600" x-text="myStats.my_assigned">{{ $myStats['total_assigned'] ?? 0 }}</span>
                </div>
            </div>
            <div class="bg-gray-50 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-600">Total traitées</span>
                    <span class="text-lg font-bold text-green-600" x-text="myStats.my_processed_total">{{ $myStats['total_processed'] ?? 0 }}</span>
                </div>
            </div>
            <div class="bg-gray-50 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-600">Performance</span>
                    <span class="text-lg font-bold text-yellow-600" x-text="myStats.my_performance + '/100'">{{ $myStats['performance_rating'] ?? 0 }}/100</span>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select x-model="reportType" required class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="agent">Mes performances</option>
...
document.addEventListener('alpine:init', () => {
    Alpine.data('statisticsPage', () => ({
        ...
        lastUpdate: '',
        myStats: {
            my_processed_today: {{ $myStats['processed_today'] ?? 0 }},
            my_processed_week: {{ $myStats['processed_this_week'] ?? 0 }},
            my_avg_time: '{{ $myStats['avg_processing_time'] ?? 'N/A' }}',
            my_success_rate: {{ $myStats['success_rate'] ?? 0 }},
            my_assigned: {{ $myStats['total_assigned'] ?? 0 }},
            my_processed_total: {{ $myStats['total_processed'] ?? 0 }},
            my_performance: {{ $myStats['performance_rating'] ?? 0 }}
        },
        charts: {},
        updateInterval: null,
        ...
        async updateRealTimeStats() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    this.myStats.my_processed_today = data.processed_today || 0;
                    this.myStats.my_processed_week = data.processed_this_week || 0;
                    this.myStats.my_avg_time = data.avg_processing_time || 'N/A';
                    this.myStats.my_success_rate = data.success_rate || 0;
                    this.myStats.my_assigned = data.total_assigned || 0;
                    this.myStats.my_processed_total = data.total_processed || 0;
                    this.myStats.my_performance = data.performance_rating || 0;
        ...
        async submitReport() {
            ...
            try {
                ...
                if (response.ok) {
                    alert(`${result.message}. Fichier: ${result.filename}`);
